...
function initializeSocket() {
    // If the Socket.IO client script didn't load (e.g. the site is hosted
    // as static files on GitHub Pages with no Node/Socket.IO server behind
    // it), `io` will be undefined. Skip real-time sync gracefully instead
    // of throwing, so the rest of the app (products, cart, chatbot...)
    // still initializes normally using the default/local product data.
    if (typeof io !== 'function') {
        console.warn('Socket.IO client not available — running without real-time sync.');
        return;
    }
    ...
    try {
        ...
        socket.on('connect_error', () => {
            // No server available (e.g. static hosting) — fail silently,
            // the app keeps working with local/default product data.
        });
    } catch (err) {
        console.warn('Socket.IO initialization failed — running without real-time sync.', err);
    }
}
